...
// this will be info in a database
const users = [{
  username: 'johndoe',
  password: 'password'
}]
...
// ACTIVITY 1
 app.get('/token', (req,res) => {
  ...
  const password = req.body.password
    ...
    const token = jwt.sign({username: username}, 'SECRETSAUCE')
    res.json({success: true, token: token})
  
}) 

// this will be info in a database like postgreSQL
const accounts = [
  {name: 'Checking', balance: 5000, username: 'johndoe'},
  {name: 'Savings', balance: 1000, username: 'johndoe'}
]
...
app.post('/login', (req, res) => {
  ...
  if (authUser) {
    // generate a token for successfully auth user; return the token
    // DO NOT PUT SENSITIVE DATA INTO TOKEN
